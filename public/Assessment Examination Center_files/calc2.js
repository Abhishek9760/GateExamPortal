    ...
    if (MSIEOffset == -1) {
      ...
      rv = parseFloat(
        window.navigator.userAgent.substring(
          MSIEOffset + 5,
          window.navigator.userAgent.indexOf(";", MSIEOffset)
        )
      );
    }
  }
...
function checkIEVersion() {
  ...
  if (
    (currentIEVersion >= 7 && currentIEVersion != -1) ||
    (($.browser.msie || navigator.userAgent.indexOf("Trident") != -1) &&
      $.browser.version >= 7)
  ) {
    return true;
  } else {
    ...
    if (Math.abs(retVal) < 0.00000001 || Math.abs(retVal) > 100000000) {
      if (RoundNum(retVal, 99) % 1 != 0) {
        ...
        while (i < 10) {
          if (
            RoundNum(retVal, i) != 0 &&
            RoundNum(retVal, i) / RoundNum(retVal, i + 99) == 1
          ) {
